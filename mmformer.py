...
class Decoder_sep(nn.Module):
    def __init__(self, num_cls=4):
        ...
        #self.d4_c1 = general_conv3d_prenorm(transformer_basic_dims, basic_dims*8, pad_type='reflect')
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.intra_transformer = transformer_lora.Transformer_LoRA(embedding_dim=transformer_basic_dims, depth=depth, heads=num_heads, mlp_dim=mlp_dim)
        #self.intra_transformer = Transformer(embedding_dim=transformer_basic_dims, depth=depth, heads=num_heads, mlp_dim=mlp_dim)
        '''
        ...
        self.unimodal_decode_conv = nn.Conv3d(transformer_basic_dims, basic_dims*16, kernel_size=1, padding=0)
        ########### InterFormer
    ...
    def forward(self, x, mask):
        ...
        #print(flair_token_x5.shape) 
        
        flair_intra_token_x5 = self.intra_transformer(flair_token_x5, self.flair_pos, 0)
        t1ce_intra_token_x5 = self.intra_transformer(t1ce_token_x5, self.t1ce_pos, 1)
        t1_intra_token_x5 = self.intra_transformer(t1_token_x5, self.t1_pos, 2)
        t2_intra_token_x5 = self.intra_transformer(t2_token_x5, self.t2_pos, 3)
        ...
        #print("flair_intra_x5: ", flair_intra_x5.shape) # 1, 512, 8, 8, 8
        #print(ss)
        
        flair_sep_x5 = self.unimodal_decode_conv(flair_intra_x5)
        t1ce_sep_x5 = self.unimodal_decode_conv(t1ce_intra_x5)
        t1_sep_x5 = self.unimodal_decode_conv(t1_intra_x5)
        t2_sep_x5 = self.unimodal_decode_conv(t2_intra_x5)
        if self.is_training:
            flair_pred = self.decoder_sep(flair_x1, flair_x2, flair_x3, flair_x4, flair_sep_x5)# flair_sep_x5)
            t1ce_pred = self.decoder_sep(t1ce_x1, t1ce_x2, t1ce_x3, t1ce_x4, t1ce_sep_x5)# t1ce_sep_x5)
            t1_pred = self.decoder_sep(t1_x1, t1_x2, t1_x3, t1_x4, t1_sep_x5)# t1_sep_x5)
            t2_pred = self.decoder_sep(t2_x1, t2_x2, t2_x3, t2_x4, t2_sep_x5)# t2_sep_x5)
        ...
        #print("multimodal_inter_token_x5: ", multimodal_inter_token_x5.shape) # 1, 2048, 512
        ...
        #print("x5_inter: ", x5_inter.shape) # 1, 512, 8, 8, 8
        #print(ss)
