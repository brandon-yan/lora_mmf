...
def test_softmax(
    ...
    for i, data in enumerate(test_loader):
        ...
        try:
            ...
            h_cnt = np.int64(np.ceil((H - patch_size) / (patch_size * (1 - 0.5))))
            h_idx_list = range(0, h_cnt)
            h_idx_list = [h_idx * np.int64(patch_size * (1 - 0.5)) for h_idx in h_idx_list]
            h_idx_list.append(H - patch_size)

            w_cnt = np.int64(np.ceil((W - patch_size) / (patch_size * (1 - 0.5))))
            w_idx_list = range(0, w_cnt)
            w_idx_list = [w_idx * np.int64(patch_size * (1 - 0.5)) for w_idx in w_idx_list]
            w_idx_list.append(W - patch_size)

            z_cnt = np.int64(np.ceil((Z - patch_size) / (patch_size * (1 - 0.5))))
            z_idx_list = range(0, z_cnt)
            z_idx_list = [z_idx * np.int64(patch_size * (1 - 0.5)) for z_idx in z_idx_list]
