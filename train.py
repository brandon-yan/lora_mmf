...
def main():
    ...
    for name, para in model.named_parameters():
        if ('intra_transformer' in name and 'lora' not in name) or 'encoder' in name or 'pos' in name:
            para.requires_grad = False
    train_params = [{'params': filter(lambda p: p.requires_grad, model.parameters()), 'lr': args.lr, 'weight_decay':args.weight_decay}]
    ...
    if args.resume is not None:
        ...
        filtered_state_dict = {key: value for key, value in checkpoint['state_dict'].items() if 'decoder' not in key}
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch+1) % 100 == 0 or (epoch>=(args.num_epochs-10)):
